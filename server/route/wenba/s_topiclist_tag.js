...
module.exports = function () {
  ...
  router.use('/', function (req, res) {
    ...
    let {index, dataMode, loadNumber} = req.query;
    ...
    let getTopic = function () {
      return new Promise((resolve, reject) => {
        let SQL = `SELECT * FROM topic_table WHERE tags LIKE '%"${dataMode}"%' limit ${min},${max}`;
     ...
     * @return {Promise}
     */
    let getTagMsg = function () {
      let SQL = `SELECT * FROM tag_table WHERE tag_value = ${dataMode}`;
      return new Promise((resolve, reject)=> {
        db.query(SQL, (err, data)=> {
          if(err) {
            return reject(err)
      ...
      function Assembly() {
        setTimeout(()=> {
          ...
          if(retTags.length && retUserInfo.length && retReplys.length) {
            ...
            res.send({'result': 1, 'data': finalyData, tagMsg: tagMsg[0]}).end();
          }else{
            Assembly();
          }
        },40)
